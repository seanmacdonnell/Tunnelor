  ...
  if (mutator.FoundGameSettings() && mutator.FoundCamera() )  {
    if (!m_has_init_tunnel_been_generated || !m_has_init_middleground_been_generated) {
      ...
      if (m_camera_bottom > m_middleground_bottom) {
        //throw Tunnelour::Exceptions::unfinished_error("Down Generation not finished!");
      }
  ...
  while (m_camera_right > m_middleground_right) {
    for (std::vector<Tunnelour::Tile_Bitmap*>::iterator tunnel_edge_tile = m_right_edge_tunnel_tiles.begin(); tunnel_edge_tile !=  m_right_edge_tunnel_tiles.end(); tunnel_edge_tile++) {
      ...
      while (m_camera_right > current_tile_right) {
        Tunnelour::Tile_Bitmap* tile = Create_Tile(static_cast<int>((*tunnel_edge_tile)->GetSize().x), false);
...
void Middleground_Controller::Switch_Tileset() {
  ...
  for (std::vector<Tunnelour::Tile_Bitmap*>::iterator tile = m_middleground_tiles.begin(); tile != m_middleground_tiles.end(); ++tile) {
    Tileset middleground_tileset;
    std::list<Tileset>::iterator tileset;
...
//------------------------------------------------------------------------------
Tunnelour::Tile_Bitmap* Middleground_Controller::Create_Tile(int base_tile_size, bool is_platform) {
  Tileset middleground_tileset;
  std::list<Tileset>::iterator tileset;
