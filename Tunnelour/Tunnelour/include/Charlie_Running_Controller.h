...
#ifndef TUNNELOUR_CHARLIE_RUNNING_CONTROLLER_H_
#define TUNNELOUR_CHARLIE_RUNNING_CONTROLLER_H_
...
//  Description : Charlie_Running_Controller is a base type for controllers, they are designed
//                to read and modify the Composite_Component class (the model)
//-----------------------------------------------------------------------------
class Charlie_Running_Controller: public Avatar_State_Controller {
 public:
  //---------------------------------------------------------------------------
  // Description : Constructor
  //---------------------------------------------------------------------------
  Charlie_Running_Controller();

  //---------------------------------------------------------------------------
  // Description : Deconstructor
  //---------------------------------------------------------------------------
  virtual ~Charlie_Running_Controller();

  //---------------------------------------------------------------------------
  // Description : Initialisation function for the Charlie_Running_Controller
  //---------------------------------------------------------------------------
 ...
 private:
  ...
  const float m_wall_colliding_mid_distance;
  const float m_wall_colliding_high_y_initial_velocity;
  const float m_wall_colliding_high_x_initial_velocity;
  const float m_wall_colliding_high_distance;
  const float m_safe_falling_limit;

  void Align_New_Avatar_State();
};
}  // namespace Tunnelour
#endif  // TUNNELOUR_CHARLIE_RUNNING_CONTROLLER_H_
